...
function recibirJson(url,funcion,datos){
   var nuevoajax=this.objeto;
   var funcionJson=funcion;
   var datosajax=datos;
   nuevoajax.open("POST",url,true);
   nuevoajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
   nuevoajax.setRequestHeader("Content-length", datosajax.length);
   ...
   nuevoajax.onreadystatechange=function() { 
      if (nuevoajax.readyState==4 && nuevoajax.status==200) {
         ...
         json.parse(nuevoajax.responseText):eval("("+nuevoajax.responseText+")");
         funcionJson(json); 
         //alert("Esto jalo, shi a huevo shi a huevo");
         //funcionJson(nuevoajax.responseText); 
      }
      //else alert("HTTP error " + nuevoajax.status+" "+nuevoajax.statusText+" "+nuevoajax.readyState+" "+nuevoajax.getAllResponseHeaders());
   }	
   nuevoajax.send(datosajax);
}